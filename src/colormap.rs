...
impl ListedColorMap {
    ...
    /// all the following colormaps are from the colorcet library, and follow this naming pattern:
    /// {category}_{huesequence}_{lightnessrange}_c{meanchroma}[_s{colorshift}[_r]]
    /// where category is one of "diverging", "linear", "cyclic", "isoluminant", "rainbow"
    /// huesequence denotes the order of hues in the colormap
    /// lightnessrange is one of "5_95", etc.
    /// meanchroma is one of "c0"... "c99"
    /// _s is an optional suffix that indicates that the colormap is shifted by an amount
    /// _r is an optional suffix that indicates that the colormap is reversed
    /// the *opic* colormaps are designed for people with colorblindness
